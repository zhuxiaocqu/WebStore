...
public class ShowOrderServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// 查询所有商品信息
		List<OrderProduct> list = dao.find(currPage);
//		System.out.println(list.get(0).getProductName());
		...
		// 计算总页数
		if(count % OrderProduct.PAGE_SIZE == 0){
			// 对总页数赋值
			pages = count / OrderProduct.PAGE_SIZE;
		}else{
			// 对总页数赋值
			pages = count / OrderProduct.PAGE_SIZE + 1;
		}
		// 实例化StringBuffer
		...
		for(int i=1; i <= pages; i++){
			...
			if(i == currPage){
				...
				// 构建分页条
				sb.append("<a href='servlet/ShowOrderProductServlet?page=" + i + "'>" + i + "</a>");
			}
			// 构建分页条
			sb.append("　");
		}
		...
		//System.out.println("呵呵showGoodServletshowGoodServletshowGoodServletshowGoodServlet");
		// 转发到good.jsp页面
		request.getRequestDispatcher("/web/order.jsp").forward(request, response);
	}
